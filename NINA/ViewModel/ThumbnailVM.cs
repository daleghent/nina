        ...
        private async Task<ImageArray> LoadRaw(IProfileService profileService) {
            ...
            using (var fs = new FileStream(ImagePath.LocalPath, FileMode.Open)) {
                using (var ms = new MemoryStream()) {
                    ...
                    fs.CopyTo(ms);
                    ms.Position = 0;
                    var iarr = await converter.ConvertToImageArray(ms, new System.Threading.CancellationToken(), true, profileService.ActiveProfile.ImageSettings.HistogramResolution);
                    return iarr;
                }
            }
        }
