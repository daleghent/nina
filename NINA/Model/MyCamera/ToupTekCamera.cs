...
namespace NINA.Model.MyCamera {
    ...
    internal class ToupTekCamera : BaseINPC, ICamera {
        ...
        public async Task<IExposureData> DownloadExposure(CancellationToken token) {
            ...
            using (token.Register(() => imageReadyTCS.TrySetCanceled())) {
                using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(15))) {
                    using (cts.Token.Register(() => { Logger.Error("ToupTekCamera - No Image Callback Event received"); imageReadyTCS.TrySetResult(true); })) {
        ...
        public void StartExposure(CaptureSequence sequence) {
            ...
            Logger.Trace($"ToupTekCamera - created new downloadExposure Task with Id {imageReadyTCS.Task.Id}");
